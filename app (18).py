...
/*
  WRAPPER — now covers the FULL viewport.
  flex + align-items:center + justify-content:center
  → the panel is always geometrically centred, regardless of screen size.
  pointer-events:none on the wrapper so the HUD stays clickable if needed.
*/
#wrapper {
  position: fixed;
...
/*
  CALCULATOR PANEL
  Fixed intrinsic size with a sensible max so it never overflows the viewport.
  overflow-y:auto  → internal scroll only, never spills outside.
  overflow-x:hidden → no horizontal scroll.
  pointer-events:all re-enables interaction on the panel itself.
*/
#panel {
  pointer-events: all;
  width: min(560px, 90vw);      /* responsive: 560 px on desktop, 90 % on mobile */
  max-height: min(72vh, 680px); /* never taller than 72 % of viewport             */
  overflow-y: auto;
  overflow-x: hidden;
