...
==================================
Architecture fix:
  The HUD + panel are injected directly into Streamlit's real DOM via
  st.markdown(unsafe_allow_html=True), NOT via components.html().
  This means position:fixed and 100vh refer to the ACTUAL browser viewport,
  so the layout always fits the window with zero outer scroll.

Layout (matches screenshot):
  • HUD iframe: position:fixed, 100vw x 100vh, z-index:1, pointer-events:none
  • Panel wrapper: position:fixed, covers HUD empty rectangle
                   (left≈1%, top≈21%, width≈54%, height≈44%)
  • Panel: centred inside wrapper, 96% x 96%, internal scroll only
  • All text: 16px
  • Page: zero outer scroll, fits browser window exactly

Run:  streamlit run app.py
Req:  hud_background.html in same folder
"""
...
# ─────────────────────────────────────────────────────────────────────────────
#  Everything injected into the real Streamlit page DOM.
#  position:fixed and 100vh/100vw now refer to the ACTUAL browser viewport.
# ─────────────────────────────────────────────────────────────────────────────
st.markdown(f"""
...
/* Make Streamlit's own containers invisible / zero-size */
.stApp,
...
/* ── PAGE: 100vw × 100vh, zero outer scroll ── */
html, body {{
...
/* ── HUD IFRAME: full-screen background ── */
#hud {{
  position: fixed;
  inset: 0;
  width:  100vw;
...
/* ── WRAPPER: covers the HUD's empty black rectangle ──
   From the screenshot the rectangle occupies:
     left ≈ 1%,  top ≈ 21%,  width ≈ 54%,  height ≈ 44%
   Flex-centres the panel inside it.                      */
#calc-wrapper {{
  position: fixed;
  left:   1%;
  top:    21%;
...
/* ── PANEL: centred, 96% of wrapper, internal scroll only ── */
#calc-panel {{
  pointer-events: all;
  width:  96%;
  height: 96%;
  ...
  box-shadow: 0 0 22px rgba(0,229,204,0.42),
              inset 0 0 40px rgba(0,229,204,0.04);
  animation: panelGlow 2.5s ease-in-out infinite alternate;
  position: relative;
  scrollbar-width: thin;
  scrollbar-color: #00e5cc rgba(0,229,204,0.08);
}}
#calc-panel::-webkit-scrollbar       {{ width: 6px; }}
#calc-panel::-webkit-scrollbar-track {{ background: rgba(0,229,204,0.04); border-radius: 3px; }}
#calc-panel::-webkit-scrollbar-thumb {{ background: #00e5cc; border-radius: 3px; }}

@keyframes panelGlow {{
  from {{ box-shadow: 0 0  8px rgba(0,229,204,0.26), inset 0 0 24px rgba(0,229,204,0.03); }}
  to   {{ box-shadow: 0 0 30px rgba(0,229,204,0.68), inset 0 0 54px rgba(0,229,204,0.07); }}
}}
...
#calc-panel h1 .tri {{ color: #00e5cc; margin-right: 8px; }}
...
.sr select {{
  background: rgba(0,14,12,0.98);
  color: #00ffee;
  border: 1px solid rgba(0,229,204,0.42);
  border-radius: 2px;
...
#calc-btn:hover  {{ background: rgba(0,229,204,0.18); box-shadow: 0 0 16px rgba(0,229,204,0.52); }}
#calc-btn:active {{ background: rgba(0,229,204,0.30); }}
...
.rbox.ok {{
  background: rgba(0,56,28,0.94);
  border: 1px solid rgba(0,200,100,0.42);
...
.sp-title {{
  ...
  margin: 6px 0 4px; font-weight: bold;
}}
...
<!-- ══ JavaScript: all physics, no page reload ══ -->
<script>
const GAS_M = {{
  N2:.028013, O2:.031999, Ar:.039948, CO2:.04401, He:.0040026,
...
function toKey(f){{return f.replace(/ /g,"_").replace(/[()]/g,"").replace(/\\//g,"").replace(/\\u00b0/g,"deg").replace(/°/g,"deg");}}
...
# Keep Streamlit alive (empty placeholder — all UI is in the markdown above)
st.empty()
