...
Architecture:
  ...
  • The iframe is sized to the real browser window using a ResizeObserver on the parent
  • Panel sits inside the HUD empty rectangle (left≈1%, top≈21%, width≈54%, height≈44%)
...
/* HUD: full-screen background layer */
#hud {
  position: fixed;
  inset: 0;
  width: 100%;
...
/*
  ...
  Measured from the HUD screenshot:
    left  ≈ 1%,   top    ≈ 21%
    width ≈ 54%,  height ≈ 44%
...
/*
  CALCULATOR PANEL
  96% of wrapper in both axes.
  overflow-y:auto  → internal scroll only, never spills outside.
...
# The CSS on the parent page (injected above via st.markdown) forces this
# iframe to be position:fixed, 100vw x 100vh, so it truly fills the browser.
